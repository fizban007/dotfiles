...
** Line Numbers
For Emacs 26 and later, =display-line-numbers-mode= is available. This is
what we use here. For reference see
...
** Theme
There are a few very good emacs themes that are downloaded. We use one
of them here, but one can choose from them at any time.
...
* Flycheck mode
=flycheck= is a automatic syntax checking mode
#+BEGIN_SRC emacs-lisp :tangle no
...
The builtin syntax highlighting doesn’t do a very good job when it
comes to modern C++. Luckily there’s a remedy:
=modern-cpp-font-lock=. Configure using:
#+BEGIN_SRC emacs-lisp
...
We try out =ccls= to do autocompletion
#+BEGIN_SRC emacs-lisp
...
(use-package ccls :ensure t
             :hook ((c-mode c++-mode objc-mode cuda-mode) .
                    (lambda ()
	                  ...
	                  (lsp)
                      (general-define-key
	                   :keymaps '(c++-mode-map cuda-mode-map)
...
:hook ((c-mode c++-mode objc-mode cuda-mode) .
         (lambda ()
           (add-to-list 'eglot-server-programs '(cuda-mode . ("ccls" "--log-file=/tmp/ccls.log")))
	   (eglot-ensure)
	   ;; (flymake-mode -1)
	   ))
)
...
Use =clang-format= from system clang
#+BEGIN_SRC emacs-lisp
...
* Python
  We mostly follow this guide to setup python IDE: https://realpython.com/emacs-the-best-python-editor/
#+BEGIN_SRC emacs-lisp
...
* Web Mode
This is for editing html presentations. There are more customization
options outlined at [[web-mode.org]]
#+BEGIN_SRC emacs-lisp
...
* Neotree
A Emacs tree plugin like NerdTree for Vim. Official repo is at https://github.com/jaypei/emacs-neotree
#+BEGIN_SRC emacs-lisp
...
* Other Misc Modes
=wgrep= enables changing the ag search result directly, which is handy
for mass search replace.
#+BEGIN_SRC emacs-lisp
...
Lua-mode is mostly for editing awesome window manager configurations
#+BEGIN_SRC emacs-lisp
...
PKGBUILD mode is for editting the archlinux package files
#+BEGIN_SRC emacs-lisp
