...
;;;###autoload
(defun +cpp-sp-point-is-template-p (id action context)
...
;;;###autoload
(defun +cpp-sp-point-after-include-p (id action context)
...
;;;###autoload
(defun +cpp-c++-lineup-inclass (langelem)
...
;;;###autoload
(defun +cpp-lineup-arglist-close (langlem)
...
(defun +cpp--re-search-for (regexp)
  (save-excursion
    (save-restriction
      (save-match-data
        (widen)
...
3. Falls back to `+cpp-default-header-file-mode', if set.
          ...
          ((+cpp--re-search-for
            (let ((id "[a-zA-Z0-9_]+") (ws "[ \t\r]+") (ws-maybe "[ \t\r]*"))
              (concat "^" ws-maybe "\\(?:"
                      ...
                      "\\)")))
           (c++-mode))
          ((functionp +cpp-default-header-file-mode)
           (funcall +cpp-default-header-file-mode))
          ((c-mode)))))

(defun +cpp-resolve-include-paths ()
  (cl-loop with path = (or buffer-file-name default-directory)
           for dir in +cpp-default-include-paths
...
(defun +cpp/reload-compile-db ()
  ...
  (when (and (featurep 'irony) irony-mode)
    (+cpp|irony-init-compile-options)))
...
(defvar +cpp--project-includes-alist nil)
;;;###autoload
(defun +cpp|init-irony-compile-options ()
...
`+cpp-default-compiler-options' and `+cpp-default-include-paths'.
  ...
  (when (memq major-mode '(c-mode c++-mode objc-mode))
    ...
    (unless (irony-cdb-autosetup-compile-options)
      (let ((project-root (doom-project-root))
            (include-paths (+cpp-resolve-include-paths)))
        (setf (alist-get project-root +cpp--project-includes-alist)
              include-paths)
        (irony-cdb--update-compile-options
         (append (delq nil (cdr-safe (assq major-mode +cpp-default-compiler-options)))
                 (cl-loop for path in include-paths
                          collect (format "-I%s" path)))
         project-root)))))

;; ;;;###autoload
;; (defun +cpp|init-ccls-compile-options ()
;;   "TODO"
;;   (when (memq major-mode '(c-mode c++-mode objc-mode))
;;     (when-let* ((include-paths (+cpp-resolve-include-paths)))
;;       (let ((args (delq nil (cdr-safe (assq major-mode +cpp-default-compiler-options)))))
;;         (setf (alist-get (or (lsp-workspace-root)
;;                              (lsp--suggest-project-root)
;;                              (doom-project-root))
;;                          +cpp--project-includes-alist)
;;               include-paths)
...
;;;###autoload
(defun +cpp|init-ffap-integration ()
...
header files."
  (when-let* ((project-root (or (bound-and-true-p irony--working-directory)
    ...
    (make-local-variable 'ffap-c++-path)
    (cl-loop for dir in (or (cdr (assoc project-root +cpp--project-includes-alist))
                            (+cpp-resolve-include-paths))
